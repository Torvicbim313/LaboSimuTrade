...
class whalesDataAndPricesWeth130 {
  static async getAll() {
    const db = await pool.getConnection();
    const query = 'SELECT * FROM trading_eth_data_130';
    try {
  ...
  static async getPaginated(offset, limit) {
    const db = await pool.getConnection();;
    const query = 'SELECT * FROM trading_eth_data_130 ORDER BY ID DESC LIMIT ? OFFSET ?';
    try {
  ...
  static async updateById(id, data) {
    const db = await pool.getConnection();;
    const query = "UPDATE trading_eth_data_130 SET ? WHERE ID = ?";
    try {
  ...
  static async getLast() {
    const db = await pool.getConnection();;
    const query = "SELECT * FROM trading_eth_data_130 ORDER BY ID DESC LIMIT 1";
    try {
      const [rows] = await db.query(query);
...
export default whalesDataAndPricesWeth130
