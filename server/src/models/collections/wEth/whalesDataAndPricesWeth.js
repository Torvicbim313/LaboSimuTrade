...
class whalesDataAndPricesWeth {
  static async getAll() {
    const db = await pool.getConnection();
    const query = 'SELECT * FROM trading_eth_data';
    try {
  ...
  static async getPaginated(offset, limit) {
    const db = await pool.getConnection();;
    const query = 'SELECT * FROM trading_eth_data ORDER BY ID DESC LIMIT ? OFFSET ?';
    try {
  ...
  static async updateById(id, data) {
    const db = await pool.getConnection();;
    const query = "UPDATE trading_eth_data SET ? WHERE ID = ?";
    try {
  ...
  static async getLast() {
    const db = await pool.getConnection();;
    const query = "SELECT * FROM trading_eth_data ORDER BY ID DESC LIMIT 1";
    try {
      const [rows] = await db.query(query);
...
export default whalesDataAndPricesWeth
