...
export async function quoteToSellWeth() {
  ...
  try {
    // Fuerza el orden correcto: tokenIn = WETH, tokenOut = USDC
    const quotedAmountOut = await quoterContract.callStatic.quoteExactInputSingle(
      CurrentConfig.tokens.in.address,   // WETH (token que vendes)
      CurrentConfig.tokens.out.address,  // USDC (token que recibes)
      poolConstants.fee,
      amountIn,
      0
    );
    console.log('\x1b[33m1 WETH/USDC en UNISWAP:',"\x1b[32m", toReadableAmount(quotedAmountOut, CurrentConfig.tokens.out.decimals),"\x1b[0m");
