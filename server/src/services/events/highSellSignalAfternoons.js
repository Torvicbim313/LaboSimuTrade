...
const backtest = async () => {
  ...
  try {
    ...
    for (let i = 4; i < rows.length; i++) {
      ...
      // Llama a la función highSellSignalAfternoons con los datos de la ventana actual
      const shouldSell = await highSellSignalAfternoons(currentSlice);
      ...
      if (shouldSell) {
        ...
        const updateQuery = `UPDATE trading_data_afternoons SET VENTA = true WHERE ID = ?`;
...
export default highSellSignalAfternoons;
