...
const highSellSignalWeth130
 = async (currentSlice = null) => {
  let db;
  try {
    let data;
    ...
    if (currentSlice) {
      ...
      const query = `SELECT FECHA, DIFERENCIA, USDCETH, ETHUSDC 
                     FROM trading_eth_data_130 
                     ORDER BY ID DESC 
                     LIMIT 7`;
    ...
    const historicalData = {
      today: {
        ...
        wbtcBuyPrice: parseFloat(data[data.length - 1].USDCETH),
        wbtcSellPrice: parseFloat(data[data.length - 1].ETHUSDC),
      },
      yesterday: {
        difference: parseFloat(data[data.length - 2].DIFERENCIA),
        wbtcBuyPrice: parseFloat(data[data.length - 2].USDCETH),
        wbtcSellPrice: parseFloat(data[data.length - 2].ETHUSDC),
      },
      twoDaysAgo: {
        difference: parseFloat(data[data.length - 3].DIFERENCIA),
        wbtcBuyPrice: parseFloat(data[data.length - 3].USDCETH),
        wbtcSellPrice: parseFloat(data[data.length - 3].ETHUSDC),
      },
      threeDaysAgo: {
        difference: parseFloat(data[data.length - 4].DIFERENCIA),
        wbtcBuyPrice: parseFloat(data[data.length - 4].USDCETH),
        wbtcSellPrice: parseFloat(data[data.length - 4].ETHUSDC),
      },
      fourDaysAgo: {
        difference: parseFloat(data[data.length - 5].DIFERENCIA),
        wbtcBuyPrice: parseFloat(data[data.length - 5].USDCETH),
        wbtcSellPrice: parseFloat(data[data.length - 5].ETHUSDC),
      },
    };
...
const backtest = async () => {
  ...
  try {
    ...
    const query = `SELECT ID, FECHA, DIFERENCIA, USDCETH, ETHUSDC 
                   FROM trading_eth_data_130 
                   ORDER BY ID ASC`;
    ...
    for (let i = 4; i < rows.length; i++) {
      ...
      // Llama a la función highSellSignalWeth130
      //  con los datos de la ventana actual
      const shouldSell = await highSellSignalWeth130
      (currentSlice);
      ...
      if (shouldSell) {
        ...
        const updateQuery = `UPDATE trading_eth_data_130 SET VENTA = true WHERE ID = ?`;
...
export default highSellSignalWeth130
;
