...
const sell6_2wEth = async () => {

  const db = await pool.getConnection();
  
  try {

    // Obtener el último valor de eth de '6_2tardes_ballenas'
    const [ethAmountResult] = await db.query(
      "SELECT eth_amount FROM 6_2weth ORDER BY id DESC LIMIT 1"
    );
    ...
    console.error('Error intentando vender y registrar en 6_2tardes_ballenas:', error);
  } finally {
    db.release();  // IMPORTANTE: liberar la conexión después de usarla
}
};

export default sell6_2wEth;
