...
import stopLoss6_3wEth from "./stopLoss6_3wEth.js";
import takeProfit6_3wEth from "./takeProfit6_3wEth.js";
...
const buy6_3wEth = async () => {

    const db = await pool.getConnection();;
    try {


        // Obtener el último registro de la tabla 6_3tardes_ballenas
        ...
        if (!lastRow) {
            console.error("No hay registros previos en 6_3weth.");
            return;
        }
        ...
        if (previousWethAmount * 1.06 < ethItCanPurchase && ethItCanPurchase < previousWethAmount * 3) {
            ...
            await takeProfit6_3wEth(ethItCanPurchase, buyPriceethUni, sellPriceethUni);
        ...
        if (previousWethAmount > ethItCanPurchase * 1.03) {
            ...
            await stopLoss6_3wEth(ethItCanPurchase, buyPriceethUni, sellPriceethUni);
        }
        ...
        // const query = `
        //     INSERT INTO 6_3tardes_ballenas (fecha, eth, usdc, accion, ballenas, precio_compra, precio_venta)
        //     VALUES (?, NULL, ?, ?, ?, ?, ?);
        // `;

        // await db.query(query, [fecha, eth, usdc, accion, ballenas, precio_compra, precio_venta]);

        // console.log("Compra registrada en 6_3tardes_ballenas:", { fecha, usdc, accion, ballenas, precio_compra, precio_venta });
    } catch (error) {
        console.error("Error intentando registrar compra en 6_3weth:", error);
    } finally {
