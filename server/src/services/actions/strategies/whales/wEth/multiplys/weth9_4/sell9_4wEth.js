...
const sell9_4wEth = async () => {

  const db = await pool.getConnection();
  
  try {

    // Obtener el último valor de eth de '9_4tardes_ballenas'
    const [ethAmountResult] = await db.query(
      "SELECT eth_amount FROM 9_4weth ORDER BY id DESC LIMIT 1"
    );
    ...
    console.error('Error intentando vender y registrar en 9_4tardes_ballenas:', error);
  } finally {
    db.release();  // IMPORTANTE: liberar la conexión después de usarla
}
};

export default sell9_4wEth;
