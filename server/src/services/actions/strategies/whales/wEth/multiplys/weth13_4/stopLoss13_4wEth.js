...
const stopLoss13_4wEth = async (amountToBuy, buyPrice, sellPrice) => {
  ...
  const db = await pool.getConnection();
    ...
    try {
        ...
        console.log("Compra en perdidas registrada correctamente en 13_4weth:", result);
        return amountToBuy;

    } catch (error) {
        console.error("Error intentando comprar en perdidas en stopLoss13_4wEth: ",error)
    } finally {
      db.release();  // IMPORTANTE: liberar la conexión después de usarla
  }
}

export default stopLoss13_4wEth;