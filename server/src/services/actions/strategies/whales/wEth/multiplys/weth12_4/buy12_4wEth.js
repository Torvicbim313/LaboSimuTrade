...
import stopLoss12_4wEth from "./stopLoss12_4wEth.js";
import takeProfit12_4wEth from "./takeProfit12_4wEth.js";
...
const buy12_4wEth = async () => {

    const db = await pool.getConnection();;
    try {


        // Obtener el último registro de la tabla 12_4tardes_ballenas
        ...
        if (!lastRow) {
            console.error("No hay registros previos en 12_4weth.");
            return;
        }
        ...
        if (previousWethAmount * 1.12 < ethItCanPurchase && ethItCanPurchase < previousWethAmount * 3) {

            // console.log('take profit')
            await takeProfit12_4wEth(ethItCanPurchase, buyPriceethUni, sellPriceethUni);
        ...
        if (previousWethAmount > ethItCanPurchase * 1.04) {
            ...
            await stopLoss12_4wEth(ethItCanPurchase, buyPriceethUni, sellPriceethUni);
        }
        ...
        // const query = `
        //     INSERT INTO 12_4tardes_ballenas (fecha, eth, usdc, accion, ballenas, precio_compra, precio_venta)
        //     VALUES (?, NULL, ?, ?, ?, ?, ?);
        // `;

        // await db.query(query, [fecha, eth, usdc, accion, ballenas, precio_compra, precio_venta]);

        // console.log("Compra registrada en 12_4tardes_ballenas:", { fecha, usdc, accion, ballenas, precio_compra, precio_venta });
    } catch (error) {
        console.error("Error intentando registrar compra en 12_4weth:", error);
    } finally {
