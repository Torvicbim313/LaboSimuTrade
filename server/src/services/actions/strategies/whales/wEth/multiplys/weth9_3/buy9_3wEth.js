...
import stopLoss9_3wEth from "./stopLoss9_3wEth.js";
import takeProfit9_3wEth from "./takeProfit9_3wEth.js";
...
const buy9_3wEth = async () => {

    const db = await pool.getConnection();;
    try {


        // Obtener el último registro de la tabla 9_3tardes_ballenas
        ...
        if (!lastRow) {
            console.error("No hay registros previos en 9_3weth.");
            return;
        }
        ...
        if (previousWethAmount * 1.09 < ethItCanPurchase && ethItCanPurchase < previousWethAmount * 3) {

            // console.log('take profit')
            await takeProfit9_3wEth(ethItCanPurchase, buyPriceethUni, sellPriceethUni);
        ...
        if (previousWethAmount > ethItCanPurchase * 1.03) {
            ...
            await stopLoss9_3wEth(ethItCanPurchase, buyPriceethUni, sellPriceethUni);
        }
        ...
        // const query = `
        //     INSERT INTO 9_3tardes_ballenas (fecha, eth, usdc, accion, ballenas, precio_compra, precio_venta)
        //     VALUES (?, NULL, ?, ?, ?, ?, ?);
        // `;

        // await db.query(query, [fecha, eth, usdc, accion, ballenas, precio_compra, precio_venta]);

        // console.log("Compra registrada en 9_3tardes_ballenas:", { fecha, usdc, accion, ballenas, precio_compra, precio_venta });
    } catch (error) {
        console.error("Error intentando registrar compra en 9_3weth:", error);
    } finally {
