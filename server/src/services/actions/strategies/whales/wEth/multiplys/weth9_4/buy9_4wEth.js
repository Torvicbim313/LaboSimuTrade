...
import stopLoss9_4wEth from "./stopLoss9_4wEth.js";
import takeProfit9_4wEth from "./takeProfit9_4wEth.js";
...
const buy9_4wEth = async () => {

    const db = await pool.getConnection();;
    try {


        // Obtener el último registro de la tabla 9_4tardes_ballenas
        ...
        if (!lastRow) {
            console.error("No hay registros previos en 9_4weth.");
            return;
        }
        ...
        if (previousWethAmount * 1.09 < ethItCanPurchase && ethItCanPurchase < previousWethAmount * 3) {
            ...
            await takeProfit9_4wEth(ethItCanPurchase, buyPriceethUni, sellPriceethUni);
        ...
        if (previousWethAmount > ethItCanPurchase * 1.04) {
            ...
            await stopLoss9_4wEth(ethItCanPurchase, buyPriceethUni, sellPriceethUni);
        }
        ...
        // const query = `
        //     INSERT INTO 9_4tardes_ballenas (fecha, eth, usdc, accion, ballenas, precio_compra, precio_venta)
        //     VALUES (?, NULL, ?, ?, ?, ?, ?);
        // `;

        // await db.query(query, [fecha, eth, usdc, accion, ballenas, precio_compra, precio_venta]);

        // console.log("Compra registrada en 9_4tardes_ballenas:", { fecha, usdc, accion, ballenas, precio_compra, precio_venta });
    } catch (error) {
        console.error("Error intentando registrar compra en 9_4weth:", error);
    } finally {
