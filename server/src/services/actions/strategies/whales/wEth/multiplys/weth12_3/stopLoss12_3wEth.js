...
const stopLoss12_3wEth = async (amountToBuy, buyPrice, sellPrice) => {
  ...
  const db = await pool.getConnection();
    ...
    try {
        ...
        console.log("Compra en perdidas registrada correctamente en 12_3weth:", result);
        return amountToBuy;

    } catch (error) {
        console.error("Error intentando comprar en perdidas en stopLoss12_3wEth: ",error)
    } finally {
      db.release();  // IMPORTANTE: liberar la conexión después de usarla
  }
}

export default stopLoss12_3wEth;