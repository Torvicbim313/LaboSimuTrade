...
import stopLoss6_2wEth from "./stopLoss6_2wEth.js";
import takeProfit6_2wEth from "./takeProfit6_2wEth.js";
...
const buy6_2wEth = async () => {

    const db = await pool.getConnection();;
    try {


        // Obtener el último registro de la tabla 6_2tardes_ballenas
        ...
        if (!lastRow) {
            console.error("No hay registros previos en 6_2weth.");
            return;
        }
        ...
        if (previousWethAmount * 1.06 < ethItCanPurchase && ethItCanPurchase < previousWethAmount * 3) {

            // console.log('take profit')
            await takeProfit6_2wEth(ethItCanPurchase, buyPriceethUni, sellPriceethUni);
        ...
        if (previousWethAmount > ethItCanPurchase * 1.02) {
            ...
            await stopLoss6_2wEth(ethItCanPurchase, buyPriceethUni, sellPriceethUni);
        }
        ...
        // const query = `
        //     INSERT INTO 6_2tardes_ballenas (fecha, eth, usdc, accion, ballenas, precio_compra, precio_venta)
        //     VALUES (?, NULL, ?, ?, ?, ?, ?);
        // `;

        // await db.query(query, [fecha, eth, usdc, accion, ballenas, precio_compra, precio_venta]);

        // console.log("Compra registrada en 6_2tardes_ballenas:", { fecha, usdc, accion, ballenas, precio_compra, precio_venta });
    } catch (error) {
        console.error("Error intentando registrar compra en 6_2weth:", error);
    } finally {
