...
const takeProfit9_2wEth = async (amountToBuy, buyPrice, sellPrice) => {
  ...
  const db = await pool.getConnection();
    ...
    try {
        ...
        const [whalesAmountResult] = await db.query(
          ...
          const query = `
          INSERT INTO 9_2weth (fecha, eth_amount, usdc, accion, ballenas, precio_compra, precio_venta)
          VALUES (?, ?, ?, ?, ?, ?, ?)
        `;
        ...
        console.log("Compra en beneficios registrada correctamente en 9_2weth:", result);
        return amountToBuy;

    } catch (error) {
        console.error("Error intentando comprar en ganancias en takeProfit9_2wEth: ",error)
    } finally {
      db.release();  // IMPORTANTE: liberar la conexión después de usarla
  }
}

export default takeProfit9_2wEth;