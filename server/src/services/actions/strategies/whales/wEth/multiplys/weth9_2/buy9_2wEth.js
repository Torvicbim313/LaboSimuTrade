...
import stopLoss9_2wEth from "./stopLoss9_2wEth.js";
import takeProfit9_2wEth from "./takeProfit9_2wEth.js";
...
const buy9_2wEth = async () => {

    const db = await pool.getConnection();;
    try {


        // Obtener el último registro de la tabla 9_2tardes_ballenas
        ...
        if (!lastRow) {
            console.error("No hay registros previos en 9_2weth.");
            return;
        }
        ...
        if (previousWethAmount * 1.09 < ethItCanPurchase && ethItCanPurchase < previousWethAmount * 3) {
            ...
            await takeProfit9_2wEth(ethItCanPurchase, buyPriceethUni, sellPriceethUni);
        ...
        if (previousWethAmount > ethItCanPurchase * 1.02) {
            ...
            await stopLoss9_2wEth(ethItCanPurchase, buyPriceethUni, sellPriceethUni);
        }
        ...
        // const query = `
        //     INSERT INTO 9_2tardes_ballenas (fecha, eth, usdc, accion, ballenas, precio_compra, precio_venta)
        //     VALUES (?, NULL, ?, ?, ?, ?, ?);
        // `;

        // await db.query(query, [fecha, eth, usdc, accion, ballenas, precio_compra, precio_venta]);

        // console.log("Compra registrada en 9_2tardes_ballenas:", { fecha, usdc, accion, ballenas, precio_compra, precio_venta });
    } catch (error) {
        console.error("Error intentando registrar compra en 9_2weth:", error);
    } finally {
