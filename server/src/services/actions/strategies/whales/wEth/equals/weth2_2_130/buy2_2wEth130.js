...
import takeProfit2_2weth130 from "./takeProfit2_2wEth130.js";
import stopLoss2_2weth130 from "./stopLoss2_2wEth130.js";
...
const buy2_2weth130 = async () => {
    ...
    try {
        ...
        if (!lastRow) {
            console.error("No hay registros previos en 2_2weth130.");
            return;
        }
        ...
        if (previousWethAmount * 1.02 < ethItCanPurchase && ethItCanPurchase < previousWethAmount * 3) {
            ...
            await takeProfit2_2weth130(ethItCanPurchase, buyPriceethUni, sellPriceethUni);
        ...
        if (previousWethAmount > ethItCanPurchase * 1.02) {
            ...
            await stopLoss2_2weth130(ethItCanPurchase, buyPriceethUni, sellPriceethUni);
        }
        ...
        // const query = `
        //     INSERT INTO 2_2tardes_ballenas (fecha, eth, usdc, accion, ballenas, precio_compra, precio_venta)
        //     VALUES (?, NULL, ?, ?, ?, ?, ?);
        // `;

        // await db.query(query, [fecha, eth, usdc, accion, ballenas, precio_compra, precio_venta]);

        // console.log("Compra registrada en 2_2tardes_ballenas:", { fecha, usdc, accion, ballenas, precio_compra, precio_venta });
    } catch (error) {
        console.error("Error intentando registrar compra en 2_2weth130:", error);
    } finally {
        db.release();  // IMPORTANTE: liberar la conexión después de usarla
    }
};

export default buy2_2weth130;
