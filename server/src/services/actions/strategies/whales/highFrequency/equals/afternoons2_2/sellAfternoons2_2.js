...
const sellAfternoons2_2 = async () => {
  try {
    ...
    // Obtener el último precio de venta desde 'trading_data'
    const [btcSellPriceResult] = await db.query(
      "SELECT PRECIO_VENTA FROM trading_data ORDER BY id DESC LIMIT 1"
    );
    ...
    // Obtener el último valor de ballenas desde 'trading_data' (campo BTC)
    ...
    // Obtener el último precio de compra desde 'trading_data'
    const [btcBuyPriceResult] = await db.query(
      "SELECT PRECIO_COMPRA FROM trading_data ORDER BY id DESC LIMIT 1"
    );
    ...
    const [result] = await db.query(query, [
      ...
      usdtAfterSell,
      'SELL',
      whalesAmount, // Ahora se obtiene de trading_data.BTC
      btcBuyPrice,
      btcSellPrice
    ]);
